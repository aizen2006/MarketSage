...
| Data stores | PostgreSQL 16, Pinecone (integrated-embedding vector store for RAG) |
...
Both backend images use `oven/bun:1`, install the monorepo, generate the Prisma client, and run migrations on startup. Provide the agent and vector-store secrets (`OPENAI_API_KEY`, `FMP_API_KEY`, `TAVILY_API_KEY`, `PINECONE_API_KEY`, `PINECONE_INDEX`) through the environment.
